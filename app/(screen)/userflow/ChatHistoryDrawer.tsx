...
type Props = {
  ...
  onContinueChat?: (
    ...
    meta?: { rawPrompt?: any; item?: HistoryItem }
  ) => void;
  onStartNewThread?: (
    messages: ParsedMessage[],
    meta?: { rawPrompt?: any; item?: HistoryItem }
...
// Currently unused, but kept if you want explicit string → JSON conversion later
const convertPromptString = (promptString: any) => {
  try {
    // Step 1: Replace role=user → "user", role=assistant → "assistant"
    let fixed = String(promptString)
      .replace(/role=/g, '"role":"')
...
function getFirstUser(parsed: ParsedMessage[]) {
  ...
  return (
    parsed.find((m) => m.role?.toLowerCase() === "user") ?? null
  );
}
function getLastUser(parsed: ParsedMessage[]) {
  if (!parsed || parsed.length === 0) return null;
  return (
    [...parsed].reverse().find((m) => m.role?.toLowerCase() === "user") ??
    null
  );
}
...
const ChatHistoryDrawer: React.FC<Props> = ({
  ...
  const apiUrl = 
  `https://meta.oxyloans.com/api/ai-service/agent/getUserHistory/${userId}/${agentId}`;
  ...
  const fetchHistory = useCallback(async () => {
    ...
    try {
      ...
      // console.log("fetchHistory response:", res.data);
      setRawResp(res.data);
      ...
      console.warn("fetchHistory error:", err.response);
      if (err.response) {
  ...
  const panResponder = useRef(
    PanResponder.create({
      ...
      onPanResponderMove: (_, gs) => {
        ...
        const newX = Math.min(
          SCREEN_WIDTH,
          Math.max(minX, SCREEN_WIDTH - drawerWidth + dx)
        );
  ...
  const confirmRestoreFromDetail = (asNewThread = false) => {
    if (asNewThread) {
      ...
      // FIX: pass meta object, not raw value directly
      onContinueChat?.(detailThread, { rawPrompt: detailRaw });
    }
  ...
  const handleLongPress = (item: HistoryItem) => {
    ...
    const actions = [
      {
        text: "Open (show full thread)",
        onPress: () => openDetail(item),
      },
      {
        text: "Continue (restore full thread)",
        onPress: () => {
          onContinueChat?.(parsed, { rawPrompt: raw, item });
          maybeAutoSend(parsed);
          Animated.timing(translateX, {
            toValue: SCREEN_WIDTH,
            duration: 160,
            useNativeDriver: true,
      ...
      {
        text: "Copy raw prompt",
        onPress: async () => {
          await Clipboard.setStringAsync(String(raw));
          Alert.alert("Copied", "Raw prompt copied");
        },
      },
      {
        text: "Share",
        onPress: async () => {
          try {
            await Share.share({ message: String(raw) });
          } catch {
  ...
  const renderPreviewFor = (
    ...
    if (
      ...
      return { text: String(firstUser.content), parsed };
    ...
    if (
      ...
      return { text: String(last.content), parsed };
      ...
      >
        ...
        <View style={styles.cardHeader}>
          <View style={{ flex: 1 }}>
            ...
            <TouchableOpacity
              onPress={() => {
                const parsedInner = parsePrompt(item.prompt);
                onStartNewThread?.(parsedInner, {
                  rawPrompt: item.prompt,
                  item,
                });
                maybeAutoSend(parsedInner);
                Animated.timing(translateX, {
                  toValue: SCREEN_WIDTH,
                  duration: 160,
                  useNativeDriver: true,
                  ...
                  <View style={styles.center}>
                    {error === "No history found" ? (
                      <>
                        ...
                        <Text style={styles.noHistorySubtext}>Start a conversation to see your history here</Text>
                      </>
                    ) : (
                  ...
                  <View style={styles.debugBox}>
                    ...
                    <ScrollView style={{ maxHeight: 180 }}>
                      <Text style={styles.debugText}>
                        {rawResp
                          ? JSON.stringify(rawResp, null, 2).slice(
                              0,
                              2000
                            )
                          : "–"}
          ...
          <View style={styles.detailCard}>
            <View style={styles.detailHeader}>
              ...
              >
                <TouchableOpacity
                  onPress={async () => {
                    await Clipboard.setStringAsync(
                      JSON.stringify(detailThread, null, 2)
                    );
                    Alert.alert("Copied", "Full thread copied");
                  }}
                  style={{ marginRight: 8 }}
                >
                    ...
                    try {
                      await Share.share({
                        message: JSON.stringify(
                          { thread: detailThread, raw: detailRaw },
                          null,
                          2
                        ),
                      });
            ...
            <View style={styles.detailFooter}>
              <TouchableOpacity
                ...
                onPress={() => confirmRestoreFromDetail(false)}
              >
              ...
              <TouchableOpacity
                ...
                onPress={() => confirmRestoreFromDetail(true)}
              >
                <Text
                  style={[
